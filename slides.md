...
| - | - |
| 14:00 ~ 14:20 | A&Dとは |
| 14:20 ~ 14:50 | A&Dのシステムを触ってみよう |
| 14:50 ~ 15:00 | 休憩 |
| 15:00 ~ 15:50 | チーム戦をやってみよう |
| 15:50 ~ 16:30 | 振り返り |
...
# インポートにエラーが出る人は、 .conf ファイルの `10.2.0.2:80` -> `10.2.0.2` にしてください
...
- ブラウザで `http://10.3.7.1:8080/` にアクセスしてみましょう
...
# 想定していた脆弱性1: SQLインジェクション

SQLインジェクション

- 攻撃:
  - `task` パラメータに `' || (SELECT flag FROM flags) || '` を入れると、FLAGが登録される
- 防御:
  ```diff
  diff --git a/repository/patchable/app.js b/repository/patchable/app.js
  index 1234567..abcdefg 100644
  --- a/repository/patchable/app.js
  +++ b/repository/patchable/app.js
...
# 想定していた脆弱性2: JWTの署名検証不備

JWTの署名検証不備

- 攻撃:
  - JWTの署名検証がされていない → 誰でも管理者APIにアクセスできる
  ```javascript
  function verifyToken(token) {
    try {
      const decoded = jwt.decode(token, { complete: true }); // ←verifyと書いてあるが、検証していない
      return decoded.payload;
      ...
  }
  ```
  - `roles` クレームに `admin` を入れたJWTを作成して、`/admin/flag` にアクセス
- 防御:
  - ちゃんとJWTの署名を検証する
  ```diff
  diff --git a/repository/patchable/apiRoutes.js b/repository/patchable/apiRoutes.js
  ...
...
# 想定していた脆弱性3: 管理者APIのアクセス

管理者APIのアクセス（クソ問です）

- 攻撃:
  - 管理者APIにアクセスするためのハッシュ値が出力される仕様(なんで？？？)
  ```javascript
  return res.status(401).send(`Access denied. Please provide valid authentication. Hash: ${ADMIN_HASH}`);
  ```
  - ハッシュ値の先頭5桁が一致すれば管理者APIにアクセスできる(なんで？？？)
  ```javascript
  if (secretHash.substring(0, 5) === ADMIN_HASH.substring(0, 5)) {
    return true;
  }
  ```
  - ハッシュ値の先頭5桁が一致する文字列を総当りして、管理者APIにアクセス → /flag1, /flag2 を奪取
- 防御:
  - レスポンスを変更するとSLA減点になってしまうので、認証のところだけ修正
  ```diff
  diff --git a/repository/patchable/adminRoutes.js b/repository/patchable/adminRoutes.js
  ...
  -  if (secretHash.substring(0, 5) === ADMIN_HASH.substring(0, 5)) {
