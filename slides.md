...
- 2023年は、TWN48というアイドルグループとして他のチームよりもチャンネル登録者数やスパチャを集めるというテーマ
- 当時の蔡英文総統も見に来た
...
curl --json '{"flags":["ICC{THIS_IS_A_FLAG}"]}' \
 ...
 https://icc-ad/core.v1.FrontendService/SubmitFlags
```
...
- **中身を確認**:
  ...
  - `docker-compose.yml`: ローカルでの再現・確認に使用
  - その他ソースコード: 脆弱性を探すためのヒント
...
- **提出コマンド**:
  ...
  ```
- **注意点**:
  - FLAGには有効期限（5分）があります。取ったらすぐ出しましょう！
  - 重複提出や自チームのFLAG提出はエラーになります。
...
- **修正と反映**:
  ...
  2. `git commit` & `push`
  3. CI/CD（ビルドシステム！）が動き、**次以降のTick**で反映される
- **SLAに注意**:
  - 修正でサービスが止まると、SLA減点の対象になります。
  - **検証環境 (`10.4.x.y`)** で動作確認してから本番にPushしましょう。
...
<div class="text-xs opacity-60 mb-2">https://github.com/eciavatta/caronte</div>

- PCAPを取り込みTCPフローを再構築・可視化
- 正規表現でフラグや攻撃パターンを検出
- サービス・ポートごとにフィルタリング
- MongoDBバックエンド＋Web UI
...
<div class="text-xs opacity-60 mb-2">https://github.com/OpenAttackDefenseTools/tulip</div>

- フローをブラウザで閲覧・検索
- 攻撃を再現するPythonスニペットを自動生成
- Suricataのアラートと連携してフローをハイライト
- フラグの検出・タグ付け機能
